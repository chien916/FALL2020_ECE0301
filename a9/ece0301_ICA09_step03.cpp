...
SortableArray readArray(ifstream& ifs,ofstream& ofs) {//Write a function that is outside the definition of your sortable array class that reads an array from a text file.
...
int main() {//Write a main function that:
	...
	unsigned int middlepoint ;//you have to manually change the start of the second sub - array in your main program
	if (ins.accessN() == 9)
		middlepoint = 4;
	else if (ins.accessN() == 8)
		middlepoint = 5;
	else if (ins.accessN() == 11)
		middlepoint = 5;
	ins.mergeArray(ins.accessAdr(), ins2.accessAdr(), middlepoint); //Uses the first object to call the merge function to merge the elements from the first object into the second.
