...
public://Create the following public member functions for the class.
	...
	void splitArray(double* pre, double* post, unsigned int i_pre_sub_left, unsigned int i_pre_sub_right) {//Write a member function to perform splitting that does not include the recursive function
		unsigned int i_pre_mid = (i_pre_sub_right + i_pre_sub_left) / 2;//Compute the midpoint index of the array to be split, where the second sub-array
		mergeArray(pre, post, i_pre_sub_left, i_pre_mid, i_pre_sub_right);//Call the merge function. Pass the pointer to the pre-split array as the pre-merge
