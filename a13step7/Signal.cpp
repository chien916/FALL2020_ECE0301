...
mglg.Title((v_lbl + "(t)=" + v_eqn).c_str());//Give the object a title, by using the Title member function.
...
mglg.SetOrigin(0, (smin + smax)*0.5);//Set the origin for the plot to (0,0), by using the SetOrigin member function.
}
else {
...
mglg.SetOrigin(0, (smin + smax)*0.5-2);//Set the origin for the plot to (0,0), by using the SetOrigin member function.
smax = smax + 5.2;
}
...
if((smax + smax*0.2)>=1)
mglg.Puts(mglPoint(0,smax+(smax-smin)*0.1), "Signal values");//Label both axes, by using the Puts member function
else
mglg.Puts(mglPoint(0, 1.2), "Signal values");//Label both axes, by using the Puts member function
mglg.Puts(mglPoint(tmax + tmax*0.1, 0), "Time (sec)");//Label both axes, by using the Puts member function
...
void Signal::f_fill_sgn_exponential(double A, double tao) {//Write a new member function for the signal class that fills the signal vector with values for
	...
	v_eqn = textNum(A) + " exp( -(t - "+ textNum(v_itt)+") / "+ textNum(tao)+" ) ";
}
