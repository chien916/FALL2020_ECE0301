...
int main(){//define main method
    ...
    //step12:
    if(voc=="v"||voc=="V"){
        double vsum[2]={8888,8888};//vsum to be outputted
        vsum[0]=memoryRtn[2]*memoryRtn[6]*cos(memoryRtn[3]-memoryRtn[7]);//calc product from formula 1
        vsum[1]=memoryRtn[0]*memoryRtn[4]+memoryRtn[1]*memoryRtn[5];//calc product from formula 2
        cout<<"The inner product of v1 and v2 using polar coordinates is "<<vsum[0]<<"\n";
        cout<<"The inner product of v1 and v2 using Cartesian coordinates is "<<vsum[1]<<"\n";
    }
    else{
        double zsum[6]={8888,8888,8888,8888,8888,8888};//zsum to be outputted
        zsum[0]=memoryRtn[2]*memoryRtn[6];//R1R2
        zsum[1]=memoryRtn[3]+memoryRtn[7];//theta1+theta2
        zsum[2]=memoryRtn[0]*memoryRtn[4]-memoryRtn[1]*memoryRtn[5];//(x1x2-y1y2)
        zsum[3]=memoryRtn[0]*memoryRtn[5]+memoryRtn[1]*memoryRtn[4];//(x1y2+y1x2)
        //calc converted zsum
        zsum[4]=zsum[0]*cos(zsum[1]);//conversion from formula
        zsum[5]=zsum[0]*sin(zsum[1]);//conversion from formula
        cout<<"Product, polar:          z1 * z2 = "<<zsum[0]<<" exp(j "<<zsum[1]<<")\n"//display text
            <<"Product, Cartesian:      z1 * z2 = "<<zsum[2]<<" + j "<<zsum[3]<<"\n"//display text
            <<"Polar, converted to Cartesian: z1 * z2 = "<<zsum[4]<<" + j "<<zsum[5]<<"\n";//display text
    }
