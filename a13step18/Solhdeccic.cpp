...
std::string Solhdeccic::folh_lh_v_solutionLabel_textRep() const {// must have the same name as the function in the first - order class that performs the same task.
	...
	std::string b_t = textNum(std::abs(lh_f_access_b()));
	std::string a_t = textNum(std::abs(solh_v_a));
	std::string sign1_t = "+";
	std::string sign2_t = "+";
	if (lh_f_access_b() < 0) {
		sign2_t = "-";
...
void  Solhdeccic::folh_f_fillSignals() {
	...
	double b24ac_t = std::pow(b_qs_t, 2) - 4 * a_qs_t*c_qs_t;//b^2-4*a*c-Charistic euqation
	if (b24ac_t > 0) {//If a b 2 > 4 , your function must implement the tasks from step 12
		double root1 = (-b_qs_t + std::sqrt(b24ac_t)) / (2 * a_qs_t);//Use the quadratic formula to compute the rates for the two exponential terms
		double root2 = (-b_qs_t - std::sqrt(b24ac_t)) / (2 * a_qs_t);//Use the quadratic formula to compute the rates for the two exponential terms
	...
	else if (b24ac_t < 0) {//If a b 2 < 4 , your function must fill the solution signal as described by equation (12):
		double sigma = b_qs_t/2;//Compute s = a / 2
		double omega = (std::sqrt(4 * c_qs_t - (std::pow(b_qs_t, 2))) / 2);
		double taoTemp = std::pow(sigma, -1);
		double sampleRateTemp = 100 * taoTemp;
		double cef1 = k_cef_K0;
		double cef2 = (k_cef_K1 - sigma*k_cef_K0) / omega ; 

		Signal* pointerTemp = lh_f_access_p_solution();
		delete pointerTemp;//delete old Signal
		pointerTemp = new Signal(501, sampleRateTemp, lh_f_access_t0());//actual signal with reference value
		(*pointerTemp).f_fill_sgn_exponential(1,1); 
		Signal* pointerTemp2 = new Signal(501, sampleRateTemp, lh_f_access_t0());//actual signal with reference value
		(*pointerTemp2).f_fill_sgn_sinusoid(cef1, omega, 0);
		Signal* pointerTemp3 = new Signal(501, sampleRateTemp, lh_f_access_t0());//actual signal with reference value
		(*pointerTemp3).f_fill_sgn_sinusoid(cef2, omega, -1.5708);//SINE WAVE ACTUALLY
		
		(*pointerTemp2).operator+(*pointerTemp3);//se the operators to set the solution signal equal to
		(*pointerTemp).operator*(*pointerTemp2);//se the operators to set the solution signal equal to
		delete pointerTemp2;//release memory
		delete pointerTemp3;//release memory
	}
}
#endif